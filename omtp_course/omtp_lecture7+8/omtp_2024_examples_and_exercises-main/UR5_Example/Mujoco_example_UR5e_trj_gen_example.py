...
class Test:
  ...
  def start(self):
    ...
    Tep = self.robot.fkine(qn) * sm.SE3.Trans(0.2, 0.2, 0.35)
    print(Tep)
    print(self.robot.fkine(qn))
    Trj = rtb.ctraj(self.robot.fkine(qn), Tep, 100)

    mdp= Trj[len(Trj)-1]*sm.SE3.Trans(0,0,-0.4)

    Trj.extend(rtb.ctraj(Trj[len(Trj)-1], mdp, 50))
    ...
    plt.savefig("Jvel.png")

    plt.figure(2)
    plt.plot(Tq[1])
    plt.title('joint positions')
    plt.savefig("Jpos.png")
    ...
    #calculate a joint rajectory based on the current state of the manipulator and the user modified state of the manipulator
    qtrj = rtb.jtraj(Cur, dest, 100)
