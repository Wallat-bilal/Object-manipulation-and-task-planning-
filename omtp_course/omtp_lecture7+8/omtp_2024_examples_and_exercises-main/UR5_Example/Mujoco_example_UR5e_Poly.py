...
class Test:
  ...
  def start(self):
    ...
    Traj = self.quintic_poly_trj(self.Jpos,t)
    ...
    dmax= np.ones(6)*5
    # call mstrj() function
    rq= rtb.mstraj(ttq,self.dt,0.5, dmax)


    # calculate derivatives
    dq,ddq,dddq = self.derivatives(rq.q)
    ## Plot results 
    fig, ax = plt.subplots(4)
    ax[0].plot(rq.q)
    ax[0].set_title('Joint positions')
    ...
    Jtrj = self.joint_trj_via(ttq, t)
    # calculate derivatives

    dq,ddq,dddq = self.derivatives(Jtrj)
