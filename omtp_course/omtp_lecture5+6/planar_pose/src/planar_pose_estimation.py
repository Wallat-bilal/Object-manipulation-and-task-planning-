...
class PlanarPoseEstimation():
    def __init__(self, config):
        ...
        if self.viz_pose:
            ...
            camera_info = rospy.wait_for_message(
                    self.config.cam_info_sub['topic'],
    ...
    def callback(
        ...
        """
        if self.viz_pose:
            self.viz_frame = np.frombuffer(
                image_sub.data, dtype=np.uint8).reshape(
                    image_sub.height, image_sub.width, -1)
